...
Page({
  ...
  upload: function(res) {
    ...
    const uploadTask = wx.uploadFile({
      url: api.yhsFileUpload,
      filePath: res.tempFilePaths[0],
      formData:{'fileType':'1010004'},
      name: 'file',
      success: function(res) {
        var _res = JSON.parse(res.data);
        if (_res.code === 1) {
          var url = _res.value.url
  ...
  submitFeedback: function(e) {
    ...
    let fileUrl = that.data.picUrls;
    if(that.data.picUrls){
      fileUrl = that.data.picUrls.join(',')
    }
    util.request(api.yhsFeedBack, {
  ...
  onShow: function() {
    this.setData({
      mobile:wx.getStorageSync('userInfo').userPhone
    })
  },
  onHide: function() {
    // 页面隐藏
